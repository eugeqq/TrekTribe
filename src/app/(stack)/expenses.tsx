      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <Pressable style={[styles.card, { marginTop: 16 }]} onPress={() => console.log("Ir a detalles del viaje")}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
            <Text style={styles.cardTitle}>Información general del viaje</Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View>
              <ExpenseCard expense={item} participantsById={participantsById} onEdit={onEditExpense} />
              <View style={{ flexDirection: "row", gap: 8, justifyContent: "flex-end", paddingRight: 24, marginBottom: 8 }}>
                <Pressable onPress={() => onEditExpense(item.id)}><Text style={{ color: C.accent }}>Editar</Text></Pressable>
                <Pressable onPress={() => onDeleteExpense(item.id)}><Text style={{ color: C.warning }}>Eliminar</Text></Pressable>
              </View>
            </View>
          )}
          ListEmptyComponent={<Text style={styles.mutedCenter}>¡Aún no hay gastos registrados! 💸</Text>}
        />
      </ScrollView>
